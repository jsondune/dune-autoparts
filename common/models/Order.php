...
class Order extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['order_status', 'payment_status', 'payment_method'], 'string', 'max' => 50],
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if (isset($changedAttributes['order_status']) && $this->order_status === self::STATUS_DELIVERED) {
    ...
    {
        return Payment::find()
            ->where(['order_id' => $this->id, 'order_status' => 'verified'])
            ->sum('amount') ?? 0;
    }
    ...
    public function confirm()
    {
        ...
        return $this->save(false, ['order_status', 'updated_at']);
    }
    ...
    {
        if (!in_array($this->order_status, [self::STATUS_PENDING, self::STATUS_CONFIRMED])) {
            return false;
        }

        $this->order_status = self::STATUS_PREPARING;
        return $this->save(false, ['order_status', 'updated_at']);
    }
    ...
    {
        if ($this->order_status !== self::STATUS_PREPARING) {
            return false;
        }
    ...
    {
        if ($this->order_status !== self::STATUS_SHIPPED) {
            return false;
        }

        $this->order_status = self::STATUS_DELIVERED;
        $this->delivered_at = time();
        return $this->save(false, ['order_status', 'delivered_at', 'updated_at']);
    }
    ...
    {
        if (in_array($this->order_status, [self::STATUS_DELIVERED, self::STATUS_CANCELLED])) {
            return false;
        }
...
class OrderQuery extends \yii\db\ActiveQuery
{
    ...
    {
        return $this->andWhere(['order_status' => Order::STATUS_PENDING]);
    }

    public function confirmed()
    {
        return $this->andWhere(['order_status' => Order::STATUS_CONFIRMED]);
    }

    public function preparing()
    {
        return $this->andWhere(['order_status' => Order::STATUS_PREPARING]);
    }

    public function shipped()
    {
        return $this->andWhere(['order_status' => Order::STATUS_SHIPPED]);
    }

    public function delivered()
    {
        return $this->andWhere(['order_status' => Order::STATUS_DELIVERED]);
    }

    public function cancelled()
    {
        return $this->andWhere(['order_status' => Order::STATUS_CANCELLED]);
    }
