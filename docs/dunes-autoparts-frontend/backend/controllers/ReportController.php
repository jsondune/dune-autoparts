...
class ReportController extends Controller
{
    ...
    public function actionSales()
    {
        ...
        // ชิ้นส่วนที่ขายได้
        $totalItemsSold = (new \yii\db\Query())
            ->from('{{order_item}}')
        ...
        $totalPayments = Payment::find()
            ->where(['between', 'created_at', $dateFromTs, $dateToTs])
            ->andWhere(['status' => Payment::STATUS_APPROVED])
            ->sum('amount') ?: 0;
    ...
    public function actionInventory()
    {
        ...
        $lowStock = Part::find()
            ->andWhere(['is_active' => 1])
            ->andWhere(['>', 'stock_quantity', 0])
            ->andWhere('stock_quantity <= min_stock')
            ->count();
        
        // สินค้าเกินสต็อก (มากกว่า 3 เท่าของ min_stock)
        $overStock = Part::find()
            ->andWhere(['is_active' => 1])
            ->andWhere('stock_quantity > min_stock * 3')
            ->andWhere(['>', 'min_stock', 0])
            ->count();
        ...
        if ($stockStatus === 'out') {
            ...
            $query->andWhere(['>', 'stock_quantity', 'min_stock']);
        } elseif ($stockStatus === 'over') {
            $query->andWhere('stock_quantity > min_stock * 3')
                  ->andWhere(['>', 'min_stock', 0]);
        }
    ...
    protected function getSalesByStaff($dateFromTs, $dateToTs)
    {
        return (new \yii\db\Query())
            ...
            ->from('{{order}}')
            ->leftJoin('{{user}}', '{{user}}.id = {{order}}.created_by')
            ->where(['between', '{{order}}.created_at', $dateFromTs, $dateToTs])
            ->andWhere(['<>', '{{order}}.status', Order::STATUS_CANCELLED])
    ...
    public function actionExportInventory()
    {
        ...
        foreach ($parts as $part) {
            ...
            if ($part->stock_quantity <= 0) {
                $status = 'หมดสต็อก';
            } elseif ($part->stock_quantity <= $part->min_stock) {
                $status = 'ใกล้หมด';
            }
