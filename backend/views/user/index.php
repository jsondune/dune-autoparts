...
<div class="user-index">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-4">
            <a href="<?= Url::to(['user/index', 'user_status' => User::STATUS_ACTIVE]) ?>" class="text-decoration-none">
        ...
        <div class="col-md-4">
            <a href="<?= Url::to(['user/index', 'user_status' => User::STATUS_INACTIVE]) ?>" class="text-decoration-none">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <?= Html::beginForm(['user/index'], 'get', ['class' => 'row g-3']) ?>
                ...
                <div class="col-md-3">
                    <?= Html::dropDownList('user_status', $status, [
                        '' => '-- สถานะทั้งหมด --',
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                ...
                                <img src="<?= $user->getAvatarUrl() ?>" class="rounded-circle" width="40" height="40" alt="Avatar">
                                <?php else: ?>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <?= Html::beginForm(['user/toggle-status', 'id' => $user->id], 'post', ['class' => 'd-inline']) ?>
                                        <button type="submit" class="btn btn-outline-<?= $user->user_status == User::STATUS_ACTIVE ? 'warning' : 'success' ?>" 
                                                title="<?= $user->user_status == User::STATUS_ACTIVE ? 'ปิดใช้งาน' : 'เปิดใช้งาน' ?>"
                                                onclick="return confirm('ต้องการ<?= $user->user_status == User::STATUS_ACTIVE ? 'ปิด' : 'เปิด' ?>ใช้งานผู้ใช้นี้หรือไม่?')">
                                            <i class="bi bi-<?= $user->user_status == User::STATUS_ACTIVE ? 'pause' : 'play' ?>"></i>
                                        </button>
                                    <?= Html::endForm() ?>
                                    <?php endif; ?>
                                </div>
