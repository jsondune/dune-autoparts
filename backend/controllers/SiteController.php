...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $todaySales = Order::find()
            ->where(['>=', '{{order}}.created_at', $todayStart])
        ...
        $lastMonthSales = Order::find()
            ->where(['>=', '{{order}}.created_at', $lastMonthStart])
        ...
        $pendingOrders = Order::find()
            ->where(['{{order}}.order_status' => ['pending', 'confirmed', 'preparing']])
            ->count();
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $daySales = Order::find()
                ...
                ->andWhere(['not in', 'order_status', ['cancelled']])
                ->sum('grand_total') ?? 0;
        ...
        $orderStatusSummary = [
            'pending' => Order::find()->where(['order_status' => 'pending'])->count(),
            'confirmed' => Order::find()->where(['order_status' => 'confirmed'])->count(),
            'preparing' => Order::find()->where(['order_status' => 'preparing'])->count(),
            'shipped' => Order::find()->where(['order_status' => 'shipped'])->count(),
            'delivered' => Order::find()->where(['order_status' => 'delivered'])->count(),
            'cancelled' => Order::find()->where(['order_status' => 'cancelled'])->count(),
        ];

        // === Top Selling Parts (เดือนนี้) ===
        $topSellingParts = Part::find()
            ...
            ->where(['>=', 'order.created_at', $monthStart])
            ->andWhere(['not in', 'order.order_status', ['cancelled']])
            ->groupBy('part.id')
